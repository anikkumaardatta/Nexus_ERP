    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          {/* Main Card */}
          <GlassCard className="bg-gradient-to-br from-brand-primary/10 to-transparent p-8 border-slate-200 dark:border-white/5">
             <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                  <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">Available Balance</p>
                  <h3 className="text-5xl font-display font-bold text-slate-900 dark:text-white">{formatCurrency(mockBalance.current)}</h3>
                  <div className="flex items-center gap-2 mt-4">
                    <span className="w-2 h-2 rounded-full bg-rose-500 animate-pulse" />
                    <span className="text-sm text-rose-600 dark:text-rose-400 font-medium">Recharge recommended for automation</span>
                  </div>
                </div>
                <div className="flex flex-col sm:flex-row gap-3">
                   <button className="px-8 py-3 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 transition-all flex items-center justify-center gap-2 shadow-lg shadow-indigo-600/20">
                     <CreditCard className="w-4 h-4" /> Quick Recharge
                   </button>
                   <button className="px-8 py-3 glass border-slate-200 dark:border-white/10 rounded-xl font-bold text-sm hover:bg-slate-50 dark:hover:bg-white/5 transition-all text-slate-700 dark:text-white">
                     View Invoices
                   </button>
                </div>
             </div>
          ...
          {/* History */}
          <GlassCard className="p-0 overflow-hidden border-slate-200 dark:border-white/5">
             <div className="p-6 border-b border-slate-200 dark:border-white/5 flex items-center justify-between bg-slate-50 dark:bg-white/[0.02]">
                <h3 className="font-display font-bold text-lg text-slate-900 dark:text-white flex items-center gap-2">
                  <History className="w-5 h-5 text-brand-primary" /> Transaction History
                </h3>
                <button className="text-[10px] uppercase font-bold text-slate-500 hover:text-slate-900 dark:hover:text-white transition-colors">See All Activity</button>
             </div>
                  ...
                  <div key={txn.id} className="p-4 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-white/[0.01] transition-colors">
                     <div className="flex items-center gap-4">
                        ...
                        <div>
                           ...
                           <p className="text-[10px] text-slate-500">{txn.date} • Ref: {txn.id}</p>
                        </div>
                     </div>
                     <div className={cn("text-sm font-bold", txn.type === 'recharge' ? "text-emerald-600 dark:text-emerald-400" : "text-slate-900 dark:text-white")}>
        ...
        <div className="space-y-6">
           <GlassCard className="bg-blue-500/5 dark:bg-blue-600/10 border-blue-500/20">
              <div className="flex items-center gap-3 mb-4">
                 <ShieldCheck className="w-6 h-6 text-blue-600 dark:text-blue-400" />
                 <h4 className="font-bold text-slate-900 dark:text-white">Payment Security</h4>
              </div>
              <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">All transactions are processed through encrypted gateways. We support bKash, Nagad, Mastercard, and Visa.</p>
           </GlassCard>

           <GlassCard className="border-slate-200 dark:border-white/5">
              <h4 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-4">Auto-Recharge</h4>
              <div className="flex items-center justify-between">
                 <span className="text-sm font-medium text-slate-700 dark:text-white">Disabled</span>
              ...
              <p className="text-[10px] text-slate-500 mt-4 leading-relaxed">Enable to automatically top up when balance hits zero using your primary card.</p>
           </GlassCard>

           <GlassCard className="p-4 border-slate-200 dark:border-white/5">
              <div className="flex items-center gap-3">
                 <TrendingUp className="w-5 h-5 text-emerald-600 dark:text-emerald-400" />
                 <p className="text-xs text-slate-700 dark:text-white">Monthly spend is <span className="font-bold text-emerald-600 dark:text-emerald-400">12% lower</span> than last month.</p>
              </div>
           </GlassCard>
        </div>
      </div>
