  ...
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
           <h2 className="text-3xl font-display font-black text-foreground tracking-tight">Executive Overview</h2>
           <p className="text-muted font-medium mt-1">Welcome back, Super Admin. Here's what's happening today.</p>
        </div>
        <div className="flex items-center gap-3">
           <div className="glass px-6 py-3 rounded-2xl flex items-center gap-3 border-border">
              <div className="flex flex-col">
                 <span className="text-[10px] font-black text-muted uppercase tracking-widest">Active Sellers</span>
                 <span className="text-lg font-black text-foreground">420</span>
              </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        {[
          ...
          <GlassCard key={i} className="p-5 md:p-6 relative overflow-hidden group hover:scale-[1.02] transition-all duration-300">
            ...
            <h3 className="text-2xl md:text-3xl font-black text-foreground mt-1">{stat.value}</h3>
          </GlassCard>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
        ...
        <GlassCard className="lg:col-span-2 p-5 md:p-8 overflow-hidden">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
             <div>
                <h3 className="text-lg md:text-xl font-bold text-foreground">Revenue Stream</h3>
                <p className="text-[10px] text-muted font-medium uppercase tracking-widest mt-1">Weekly Performance Overview</p>
             </div>
             <div className="flex bg-surface p-1 rounded-xl self-start sm:self-auto">
                ...
                <button className="px-4 py-1.5 text-[10px] font-black uppercase tracking-widest text-muted hover:text-foreground transition-colors">Orders</button>
             </div>
          </div>
        ...
        <GlassCard className="p-5 md:p-8">
           <h3 className="text-lg md:text-xl font-bold text-foreground mb-1">Return Analytics</h3>
           <p className="text-[10px] text-muted font-medium uppercase tracking-widest mb-8">Performance breakdown</p>
           ...
           <div className="h-48 md:h-64 relative">
             ...
             <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                <span className="text-xl md:text-2xl font-black text-foreground">4.2%</span>
              ...
              {returnData.map((item) => (
                <div key={item.name} className="flex items-center justify-between group">
                   <div className="flex items-center gap-3">
                      ...
                      <span className="text-xs md:text-sm font-bold text-muted group-hover:text-indigo-500 dark:group-hover:text-foreground transition-colors">{item.name}</span>
                   </div>
                   <span className="text-xs md:text-sm font-black text-foreground">{item.value}%</span>
                </div>
              ))}
           </div>
        </GlassCard>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
        ...
        <GlassCard className="p-5 md:p-8">
           <div className="flex items-center justify-between mb-8">
              <h3 className="text-lg md:text-xl font-bold text-foreground">Courier Performance</h3>
              ...
              {courierPerformance.map((courier) => (
                <div key={courier.courier} className="space-y-2">
                   <div className="flex justify-between items-center text-[10px] font-bold uppercase tracking-widest">
                      <span className="text-muted">{courier.courier}</span>
                      <span className="text-indigo-600 dark:text-indigo-400">{courier.rate}% Success</span>
                   </div>
                   <div className="h-1.5 md:h-2 bg-surface rounded-full overflow-hidden">
                      <motion.div 
                        initial={{ width: 0 }}
        ...
        <GlassCard className="p-5 md:p-8">
           <div className="flex items-center justify-between mb-8">
              <h3 className="text-lg md:text-xl font-bold text-foreground">Inventory Threshold</h3>
           ...
           <div className="space-y-3">
              {[
                ...
                <div key={i} className="flex items-center justify-between p-3 md:p-4 rounded-xl bg-surface border border-border group hover:bg-surface-hover transition-all">
                   <div className="flex items-center gap-3 md:gap-4">
                      ...
                      <div>
                         ...
                         <p className="text-[9px] md:text-[10px] font-mono text-muted mt-1">Stock Left: {item.stock}</p>
                      </div>
                   </div>
                   <button className="p-2 bg-surface border border-border text-indigo-500 dark:text-indigo-400 hover:text-indigo-600 dark:hover:text-white transition-all rounded-lg">
